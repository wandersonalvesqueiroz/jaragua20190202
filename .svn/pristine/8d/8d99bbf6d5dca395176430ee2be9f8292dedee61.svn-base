				...
				<tr>
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<td class="acyfield_<?php echo $oneField; ?>">
								<input id="user_phone_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $phoneCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $phoneCaption?>';"<?php } ?> class="inputbox" type="text" name="user[phone]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->phone; elseif(!$displayOutside) echo $phoneCaption; ?>" />
							</td> <?php
