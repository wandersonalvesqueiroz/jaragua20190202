...
abstract class ModJemTeaserHelper
{
	...
	public static function getList(&$params)
	{
		...
		foreach ($events as $row)
		{
            ...
            $lists[$i]->alltime     = JemOutput::formatTimeOtimize($row->times, $row->endtimes, $timeFormat);
			...
			if (empty($description)) {
                ...
                $last = strrpos($description, " ");
                $description = substr($description, 0, $last);
				$lists[$i]->eventdescription = $description . '&hellip;';
			} else {
	...
	protected static function _format_date_time($row, $method, $dateFormat = '', $timeFormat = '', $addSuffix = false)
	{
		if (empty($row->dates)) {
			...
			# datemethod show date - not shown beause there is a calendar image
				...
				if (($dates_stamp >= $today_stamp) && ($enddates_stamp > $dates_stamp) && (($enddates_stamp - $dates_stamp) != 86400)) {
				...
				#diferença de um dia
                elseif (($dates_stamp >= $today_stamp) && ($enddates_stamp > $dates_stamp) && (($enddates_stamp - $dates_stamp) == 86400)) {
                    $startdate = JemOutput::formatdate($row->dates, $dateFormat);
                    $enddate = JemOutput::formatdate($row->enddates, $dateFormat);
                    $date = JText::sprintf('MOD_JEM_TEASER_FROM_ONE_DAY', $startdate, $enddate);
                    # don't show endtime because calendar is shown
                }
				# Current multidayevent (Until 18.08.2008)
				elseif ($row->enddates && ($enddates_stamp >= $today_stamp) && ($today_stamp < $dates_stamp)) {
					$enddate = JEMOutput::formatdate($row->enddates, $dateFormat);
